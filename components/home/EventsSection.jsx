    ...
    <section className="mt-10 w-full overflow-hidden font-sans">
      ...
      <div className="relative group min-h-[400px]">
        {loading ? (
          ...
          <div
            className={`grid ${getGridColsClass()}  transition-all duration-500`}
          >
            {displayEvents.map((event) => (
              <div
                key={event._id || event.id}
                className="relative h-[400px] overflow-hidden group/item animate-fadeIn"
              >
                <Image
                  src={event.image}
                  alt={event.title}
        ...
        {!loading && !error && filteredEvents.length > 3 && (
          <>
            <button
              onClick={handlePrev}
              className="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-black/40 text-white rounded-full hover:bg-black/70"
            >
              <ChevronLeft size={30} />
            </button>
            <button
              onClick={handleNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-black/40 text-white rounded-full hover:bg-black/70"
            >
      ...
      {/* Month Timeline */}
      <div
        className="bg-[#a31d1d] py-10 px-6 relative"
        style={{
          ...
          <div className="relative z-10 flex justify-between items-center">
            {months.map((month, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex flex-col items-center gap-4 cursor-pointer min-w-[50px]"
                  onClick={() => {
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full border-[3px] border-[#f1c40f] transition-all ${
                      isSelected
